  ...
  const fetchOrders = async (email) => {
    try {
      ...
      const res = await fetch(
        `https://lgmsports.onrender.com/api/orders?email=${email}`
      );
            ...
            {loadingOrders ? (
              ...
              <div className="space-y-6">
                {orders.map((order) => (
                  <div
                    key={order.id}
                    className="relative overflow-hidden rounded-2xl border border-blue-200 bg-white shadow-lg hover:shadow-xl transition-transform transform hover:scale-[1.01]"
                  >
                    {/* Order Header: Order ID | Date & Time | Delivery Status */}
                    <div className="flex flex-col md:flex-row items-center justify-between bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4 text-white rounded-t-2xl">
                      {/* Left: Order ID */}
                      ...
                      {/* Center: Date & Time */}
                      <div className="mt-2 md:mt-0 text-sm opacity-90 bg-white text-blue-600 px-3 py-1 rounded-lg shadow-sm">
                    ...
                    <div className="p-5">
                      ...
                      {/* Discount Breakdown */}
                      <div className="bg-blue-50 rounded-xl overflow-hidden mt-4">
                        {(order.discountApplied === true ||
                          (order.couponData &&
                            Object.keys(order.couponData).length > 0) ||
                          ...
                          <>
                            ...
                            <div className="flex justify-between items-center px-4 py-2 text-sm text-gray-600">
                              ...
                              <span className="line-through">
                                ₹{Math.round(order.totalAmount / 100 / 0.9)}
                              </span>
                            </div>
                          </>
                        )}
