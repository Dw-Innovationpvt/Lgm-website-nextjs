...
export const createOrder = async (req, res) => {
  ...
  const browser = await puppeteer.launch({
    headless: "new",
    args: ["--no-sandbox", "--disable-setuid-sandbox"], // safe for Hostinger/Vercel
  });
  const page = await browser.newPage();

  // ✅ Convert both logos to Base64
  const firstLogoBase64 = getBase64FromPublic("logo.jpg");      // <-- first logo (top-right)
  const secondLogoBase64 = getBase64FromPublic("new-logo.png"); // <-- second logo (center)

  // ✅ Build HTML with same style as frontend
...
// 🔹 Helper to convert /public files to Base64
...
// 🔹 HTML template matching frontend API
